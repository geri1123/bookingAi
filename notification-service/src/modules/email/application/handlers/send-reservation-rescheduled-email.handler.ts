...
import { ReservationRescheduledEmailPayload } from "../../domain/types/email-job.types";
import { buildReservationRescheduledEmailHtml } from "../templates/reservation-rescheduled-email.template";
...
@Injectable()
export class SendReservationRescheduledEmailHandler {
  private readonly logger = new Logger(SendReservationRescheduledEmailHandler.name);

  constructor(private readonly emailSender: EmailSender) {}

  async handle(payload: ReservationRescheduledEmailPayload): Promise<void> {
    ...
    const html = buildReservationRescheduledEmailHtml({
      businessName: payload.businessName,
      customerName: payload.customerName,
      customerPhone: payload.customerPhone,
      serviceName: payload.serviceName,
      previousStartTime: formatDateTimeNumeric(payload.previousStartTime, payload.businessTimezone),
      previousEndTime: payload.previousEndTime
        ? formatDateTimeNumeric(payload.previousEndTime, payload.businessTimezone)
        : undefined,
    ...
    const subject = `Rezervim i ndryshuar nga ${payload.customerName}`;
