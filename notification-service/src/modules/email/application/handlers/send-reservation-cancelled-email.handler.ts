...
import { ReservationCancelledEmailPayload } from "../../domain/types/email-job.types";
import { buildReservationCancelledEmailHtml } from "../templates/reservation-cancelled-email.template";
...
@Injectable()
export class SendReservationCancelledEmailHandler {
  private readonly logger = new Logger(SendReservationCancelledEmailHandler.name);

  constructor(private readonly emailSender: EmailSender) {}

  async handle(payload: ReservationCancelledEmailPayload): Promise<void> {
    ...
    const html = buildReservationCancelledEmailHtml({
    ...
    const subject = `Rezervim i anulluar nga ${payload.customerName}`;
